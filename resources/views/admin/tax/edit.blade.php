    ...
    <div class="w-full px-4 md:px-0 md:mt-8 mb-16 text-gray-800 leading-normal">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="border-t border-gray-200">
                <dl>
                    {{-- Update --}}
                    <form action="{{ route('admin.tax.update', ['tax' => $tax->id]) }}" method="POST">
                        @csrf
                        @method('put')
                        ...
                        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <input type="text" name="rate" id="rate"
                                    class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300"
                                    placeholder="e.g: 0.20, 0.5" value="{{ $tax->rate }}">
                            </dd>
                        </div>
                    @if ($tax->is_active == 1)
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    @if ($tax->is_active == 1)
                    {{-- DISABLE FORM --}}
                    <form action="{{ route('admin.tax.destroy', ['tax' => $tax->id]) }}" method="POST">
                        @csrf
                        @method('delete')
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt class="text-xl font-medium text-gray-500">
                    ...
                    @else
                    <form action="{{ route('admin.tax.activate', ['tax' => $tax->id]) }}" method="POST">
                        @csrf
                        @method('put')
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt class="text-xl font-medium text-gray-500">
